...
class AuthenticationTest {
	...
	@Test
	void writeApisRequireAuthenticationButGetApisDoNot() {
		ResponseEntity<String> post = rest.exchange("/posts", HttpMethod.POST, new HttpEntity<>(new HttpHeaders()), String.class);

		assertThat(post.getStatusCode()).isEqualTo(HttpStatus.UNAUTHORIZED);
		assertThat(get("/posts", null).getStatusCode()).isNotEqualTo(HttpStatus.UNAUTHORIZED);
		assertThat(get("/posts/1", null).getStatusCode()).isNotEqualTo(HttpStatus.UNAUTHORIZED);
	}
